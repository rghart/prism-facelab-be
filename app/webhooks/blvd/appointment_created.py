...
def appointment_created():
    ...
    try:
        ...
        sheet_input_datetime = datetime.datetime.fromisoformat(
            row_data[SheetHeaderIndex.INPUT_TIME.value]
        )

        if row_data[SheetHeaderIndex.INITIAL_APPT_BOOKED.value] == "FALSE":
            row_data[SheetHeaderIndex.INITIAL_APPT_BOOKED.value] = "TRUE"
        elif (
            sheet_input_datetime
            <= current_time
            <= (sheet_input_datetime + datetime.timedelta(seconds=15))
        ):
        ...
        logger.info(
            f"NOT FOUND: Attempted to find client_id {appt_client_id} in google sheet."
        )
        return request_json
